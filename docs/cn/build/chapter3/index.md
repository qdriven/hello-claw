...
| 生命周期层 | `BOOTSTRAP.md` `BOOT.md` `HEARTBEAT.md` | 首次引导、开机清单、巡检单 | 分别对应 brand-new workspace、Gateway 启动检查、定时巡检 |
...
| `memory/` | 作为磁盘记忆层，通过 memory 工具按需读取 | 让按天积累的记录按需召回，而不是每轮都常驻 |
| `BOOTSTRAP.md` `BOOT.md` `HEARTBEAT.md` | 走三条不同的生命周期路径 | `BOOTSTRAP.md` 只在 brand-new workspace 时出现，`BOOT.md` 更像 Gateway 启动检查，`HEARTBEAT.md` 则是 heartbeat 相关上下文的一部分 |
...
更准确地说，`MEMORY.md` 现在会作为长期记忆文件进入 Project Context；真正主要靠工具按需召回的，是 `memory/YYYY-MM-DD.md` 这类按天累积的记录。  
这也是为什么 memory 这一层更像“精选常驻资料 + 可检索日记”的组合，而不是所有记忆都永远摊在桌面上。
...
- `HEARTBEAT.md` 的最佳实践一直都是“短”，但 heartbeat 的轻量化更多靠 `lightContext`、`isolatedSession` 这类机制控制；
