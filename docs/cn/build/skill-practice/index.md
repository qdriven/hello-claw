...
前面几章主要在解释 OpenClaw 为什么这样设计，以及这些设计怎样落到工具、消息循环和网关上。走到这里，最自然的下一步不是立刻去改 Gateway 核心，而是先进入一个**最常用、也最容易上手的定制入口**：把一项新能力封装成 Skill。

你可以把这一章理解成构建篇的落地收束。前面的 1-10 章是在帮你搭系统视角；这一章开始，才正式把视角从"理解 OpenClaw"切到"开始扩 OpenClaw"。

所以这一章刻意不从源码改造或渠道插件写起，而是先选 Skill 作为落点：

- 它比直接改 runtime 更轻；
- 比改渠道层更贴近日常需求；
- 同时又能把第 3 章的工作区、第 4 章的工具系统和附录 D 的 Skills 机制真正串起来。

如果说构建篇前十章主要在回答"OpenClaw 为什么这样工作"，那这一章回答的就是：**怎么把这些机制真正封装成一个可以复用的能力单元。**

它也是通往“龙虾大学”的一座桥。因为到了真实场景里，大家很少会单独讨论 prompt、tools 或 queue；更常见的问题是：**怎么把这些底层能力组合成邮箱助手、论文推送、Vibe Coding、多智能体协作这类完整能力。**
...
更稳妥的理解是：可用 Skill 往往会按会话快照，或者以延迟刷新的方式更新。也就是说，你改完 `SKILL.md` 之后，不一定当场生效。
...
写出单个 Skill，只是从“理解系统”走向“构建能力”的第一步。

真正贴近用户体验的，通常不是一个孤立的 `SKILL.md`，而是下面这类组合：
...
这也是为什么这一章会放在构建篇的最后。它既帮你把前面的机制真正落到手上，也把阅读路径自然接到“龙虾大学”的场景化案例上。

如果说：

- 构建篇前十章在讲**系统怎么工作**
- 这一章在讲**能力怎么封装**
- 那“龙虾大学”讲的就是**这些能力怎么落到真实任务**

所以读完这里，最自然的下一步就是去看“龙虾大学”：

- [龙虾大学导览](/cn/university/intro)
- 邮箱助手、论文推送、Vibe Coding、多智能体协作这类完整案例
