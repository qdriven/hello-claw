...
OpenClaw 本身已经提供了一层实用的基线防护：
- **配对码**：首次连接时需要验证
- **白名单**：限制哪些用户可以发送消息
- **权限配置**：可以禁用某些工具

如果你的使用场景属于“个人助手 / 可信操作员”模型，这些基线防护往往已经够用。  
但一旦你开始追求更强的隔离、密钥托管、审计和不信任第三方工具，这些措施的共同局限就会暴露出来：**它们主要还是运行时和策略层的防护**。

也就是说，本章讨论的不是“OpenClaw 默认完全不安全”，而是：**如果你要把 Agent 放进更高风险、更高合规要求的环境，还需要哪些更底层的防线。**
