...
这一板块聚焦"自己动手定制 OpenClaw"。这里的阅读节奏和前两板块不太一样：

- 第 1-10 章主要是在帮你建立系统视角，先看清 Agent Loop、提示词、工具、网关和不同方案的取舍；
- 到了真正要开始扩展时，最自然、也最常用的入口通常不是直接改核心源码，而是**先学会写 Skill**；
- 所以当前第三板块会直接落到一章不再继续编号的**实战章**，把它当成"从理解系统走向第一次动手扩展"的起点。
...
- 第 3 章告诉你什么应该常驻在工作区里；
- 第 4 章告诉你 Skill 最终要怎样调动工具；
- 第 8-10 章则让你看过不同系统在"能力边界"上的取舍。

所以这章在阅读体验上更像第三板块的**第一个实操落点**，也是构建篇通往“龙虾大学”的桥。内容会从描述到代码逐步展开：
...
如果你已经有一个具体的定制目标（"我想让它接入公司的XX系统"），直接跳到实战章，再配合附录 D。边读边做，在实践中有针对性地回头查原理。读完以后，再顺着它去“龙虾大学”看完整场景。
