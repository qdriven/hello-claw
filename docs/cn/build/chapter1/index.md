...
OpenClaw的使用门槛：
  ...
  安装 → 编辑 Markdown 文件 → 在你常用的聊天渠道里发消息
```

到 2026 年 4 月，OpenClaw 已经吸引了大量开发者关注，并持续出现在 GitHub 和社区讨论里。
...
OpenClaw 的 agent loop 可以说是融合了 **ReAct “边判断边行动”的思想** 与 **如今大模型在训练中内化的决定如何调用工具的能力**。
...
2. **Security模式** 和 **Ask模式** 决定这条命令允不允许、要不要先人工确认
...
```
你在 Telegram、飞书或 Web 界面里发了一条消息："帮我找出项目里所有的 console.log"
```
...
**下一步**：[第二章 Agent Loop](../chapter2/index.md)——深入探讨 Agent 的核心引擎，理解“观察-思考-行动”怎样在系统里持续跑起来。
