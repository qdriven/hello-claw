...
| 当前官方工具面 | 按 section / profile 组织的 built-in surface | `memory_search` `sessions_*` `message` `cron` `image` `tts` |
...
| 插件 | 扩展系统能力面，比如新渠道、模型提供商、工具、技能、语音、图像生成、memory / context slot |
...
| `tools.byProvider` | 不同模型提供商看到的基础工具面可不可以不同 |
| `allow` / `deny` + agent 级配置 | 全局和某个具体 Agent 要不要进一步收紧 |
| sandbox tool policy | sandboxed run 里还允许哪些工具出现 |
...
**第三类**是 `apply_patch` 这类更工程化的修改工具。它不是每个场景都会出现，也不是所有模型提供商都会支持得一样好。更准确地说，它是 `exec` 下面一个需要显式开启的实验性 subtool，主要面向 OpenAI / OpenAI Codex 模型，是对“修改文件”这一层的专业扩展，而不是修改能力的起点。
...
3. 技能来源可以有多种，通常是 bundled / managed-local / workspace 三层，plugin 也可以携带 skills，而工作区里的技能通常优先级更高。
...
**下一章：** [第五章 消息循环](../chapter5/index.md)
