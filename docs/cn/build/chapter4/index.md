...
**第三类**是 `apply_patch` 这类更工程化的修改工具。它不是每个场景都会出现，也不是所有模型提供商都会支持得一样好。按当前官方文档，它是 `exec` 下面的结构化改动 subtool，在 OpenAI / OpenAI Codex 模型上默认可用，也可以再通过 `tools.exec.applyPatch` 收紧模型范围或工作区边界。它更像是对“修改文件”这一层的专业扩展，而不是修改能力的起点。
