...
命令执行沙箱还封锁了主流构建工具链的环境变量注入路径，防止攻击者通过环境变量劫持构建过程：

| 封锁的环境变量 | 攻击向量 |
|--------------|---------|
| `MAVEN_OPTS`、`SBT_OPTS`、`GRADLE_OPTS` | JVM 启动参数注入，可加载任意类 |
| `GLIBC_TUNABLES` | glibc 调优接口，可触发本地提权 |
| `DOTNET_ADDITIONAL_DEPS` | .NET 依赖劫持，可注入任意程序集 |

这些环境变量在沙箱执行环境中被自动清除，Agent 无法通过设置它们来影响构建工具的行为。
