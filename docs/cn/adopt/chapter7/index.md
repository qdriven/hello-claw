...
> **模型标识格式**：OpenClaw 统一使用 `provider/model-name` 格式标识模型。目前支持 12+ 个官方提供商：Ollama、硅基流动、深度求索、通义千问、月之暗面、阶跃星辰、稀宇科技、火山引擎（豆包）、智谱、OpenRouter、Z.AI 等。混元、文心一言等其他提供商可通过自定义 OpenAI 兼容端点接入。
>
...
# 添加智谱（内置提供商）
openclaw config set llm.providers.glm.apiKey "sk-xxxxx"
